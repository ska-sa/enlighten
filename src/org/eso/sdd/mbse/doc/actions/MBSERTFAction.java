...
public class MBSERTFAction extends DefaultBrowserAction {

	private PropertyManager properties = null;
	private JFileChooser fc = new JFileChooser();


	public MBSERTFAction() {
		super("", "SE2:Generate XML to RTF", null, null);
	...
	public void actionPerformed(ActionEvent e) {
		...
		rtfFileName = destFile.getAbsolutePath();
		rtfFileName = rtfFileName.replaceFirst(".xml", ".rtf");
		rtfFile = new File(rtfFileName);
		...
		xsltFOP k = new xsltFOP(GENERATION_MODE.RTF_ONLY);
		...
		if (k.getStatus() == 0) {
			genUtility.displayWarning("Generated RTF to " + rtfFile.toString());
		}
		if (k.getStatus() == 1) {
