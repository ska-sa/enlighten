...
public class MBSEPdfDirectAction extends DefaultBrowserAction {

	private PropertyManager properties = null;
	private JFileChooser fc = new JFileChooser();

	public MBSEPdfDirectAction() {
		super("", "SE2:Generate PDF", null, null);
	...
	public void actionPerformed(ActionEvent e) {
		...
		Tree tree = getTree();
		Node node = null;
		FileFilter ff = new FileNameExtensionFilter("PDF file", "pdf", "[df");
		...
		// set default name
		File f = new File(book.getName().toString().trim()	+ ".pdf");
		...
		xmlFileName = pdfFile.getAbsolutePath();
		xmlFileName = xmlFileName.replaceFirst(".pdf", ".xml");
		destFile = new File(xmlFileName);

		// check files already exist and ask user
		if(pdfFile.exists() || destFile.exists()) {
		...
		xsltFOP k = new xsltFOP(GENERATION_MODE.XML_AND_PDF);
