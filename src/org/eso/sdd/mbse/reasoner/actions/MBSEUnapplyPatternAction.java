...
import org.eso.sdd.mbse.reasoner.UnapplyPatternReasoner;

import java.awt.event.ActionEvent;

@SuppressWarnings("serial")
public class MBSEUnapplyPatternAction extends DefaultBrowserAction {
	...
	public MBSEUnapplyPatternAction() {
		super("", "SE2: remove Reasoning pattern", null, null);
	...
	public void createReasoner(NamedElement ne, boolean multi, Tree tree) {
		...
		if (!SessionManager.getInstance().isSessionCreated()) {
			...
			CustomDialog dlg = new CustomDialog(
					"System Reasoner Pattern Wizard", "unapply");
			...
			if (dlg.getSelection() == 0) {
				...
				if (be != null) {
					for (BaseElement kk : be) {
						...
						UnapplyPatternReasoner jj = new UnapplyPatternReasoner();
						...
						for (Node e1 : tree.getSelectedNodes()) {
							...
							jj.removeClassifier(
									(Element) e1.getUserObject(),
									(Classifier) ((Element) kk),
									(Classifier) ((Element) e1.getUserObject()),
									factory, tmp.getResults()[0]// unapply
																// recursively
									, tmp.getResults()[1]// remove role names
                                    , tmp.getResults()[2]// remove subsetted
									, tmp.getResults()[3]// remove redef
									, tmp.getResults()[4]);// remove valuprop
