import { Component, ViewChild, Renderer, OnInit, OnDestroy, Output, EventEmitter, Input  } from '@angular/core'
...
export class CanvasDrawComponent implements OnInit {
  ...
  @Input() targetId 
  @Input() boardId
  @Input() currentColour: string = '#000' 
  ...
  handleStart(ev){
    ...
    if(this.panning !== true) {
      ...
      var mouseYT=Math.round((mouseY-this.panY - 56)/this.scaleFactor)
